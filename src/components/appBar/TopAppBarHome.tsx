...
interface Props {
  ...
  notiNumToRead?: Number;
}

const TopAppBarHome = ({ roomInviteCode, notiNumToRead }: Props) => {
  const router = useRouter();

  const [notiNum, setNotiNum] = useState<any>(0);
  const [pageYOffset, setPageYOffset] = useState<any>(0);
  const [innerWidth, setInnerWidth] = useState<any>(600);
        ...
        <AppBarDetailDiv>
          ...
          <AppBarRightDiv>
            ...
            <MenuBtnDiv onClick={() => router.push('/notification')}>
              <Badge color="primary" badgeContent={notiNum}>
...
const MenuBtn = Styled.div<{ isSelected: boolean }>`
    ${(props) =>
      ...
      `background: linear-gradient(65deg, rgba(153,196,190,1) 0%, rgba(1,99,49,1) 33%, rgba(25,153,149,1) 70%, rgba(2,129,154,1) 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;`}
    font-size: 16px;
    font-weight: 500;
    color: #333;
    word-spacing: -2px;
